...
import type { Candidate, CheckItem, EvidencePlanItem, ExternalEvidenceItem, FactItem, PomInfo, ProjectFacts, ReviewDepth, ReviewPlanItem, ScanResult, SensitiveFinding } from "./types.js";

export function scanRepo(repo: string, mode: "full" | "changed", externalEvidence: ExternalEvidenceItem[] = [], reviewDepth: ReviewDepth = "standard"): ScanResult {
  const allFiles = walkFiles(repo);
  const scopedFiles = mode === "changed" ? changedFiles(repo) : allFiles;
  const entries: Record<string, Array<{ path: string; name: string }>> = Object.fromEntries(entryTypes.map((type) => [type, []]));
...
function buildReviewPlan(evidencePlan: EvidencePlanItem[], candidates: ScanResult["candidates"], facts: ProjectFacts, externalEvidence: ExternalEvidenceItem[]): ReviewPlanItem[] {
...
function reviewFocus(category: string): string[] {
  switch (category) {
    case "cli":
...
function expectedExternalEvidenceTypes(category: string): string[] {
  switch (category) {
    case "cli":
    case "mcp":
    case "adapter":
...
function normalizeExternalEvidenceType(value: string): string {
  if (value === "code_graph" || value === "repo_map") return "architecture_overview";
  if (value === "impact" || value === "impact_analysis") return "impact_radius";
  if (value === "test_gaps" || value === "coverage_gap") return "test_gap";
  return value;
}

function reviewRiskFlags(item: EvidencePlanItem, missingExternal: string[]): string[] {
  return unique([
...
function relatedFactIds(candidate: Candidate | undefined, facts: ProjectFacts): string[] {
...
function allFactIds(facts: ProjectFacts): string[] {
  return unique([
...
function detectFacts(repo: string, files: string[], entries: Record<string, Array<{ path: string; name: string }>>): ProjectFacts {
  return {
...
function packageJsonFact(repo: string, files: string[]): ProjectFacts["package_json"] {
  const rel = files.find((item) => item === "package.json");
  if (!rel) {
    return undefined;
  }
  try {
    const parsed = JSON.parse(read(repo, rel)) as Record<string, unknown>;
    return {
      path: rel,
      name: typeof parsed.name === "string" ? parsed.name : undefined,
      bin: objectKeys(parsed.bin),
      scripts: objectKeys(parsed.scripts),
      files: Array.isArray(parsed.files) ? parsed.files.filter((item): item is string => typeof item === "string") : [],
...
function mcpToolFacts(repo: string, rel: string): FactItem[] {
  const text = read(repo, rel);
...
function adapterFacts(entries: Record<string, Array<{ path: string; name: string }>>): FactItem[] {
...
function schemaFact(repo: string, rel: string): FactItem {
  try {
    const parsed = JSON.parse(read(repo, rel)) as Record<string, unknown>;
    const required = Array.isArray(parsed.required) ? parsed.required.filter((item): item is string => typeof item === "string") : [];
...
function testSummary(rel: string): string {
  if (/cli/i.test(rel)) return "CLI regression test asset";
  if (/mcp/i.test(rel)) return "MCP regression test asset";
  return "test asset";
}
...
function detectCandidates(files: string[], entries: Record<string, Array<{ path: string; name: string }>>, externalEvidence: ExternalEvidenceItem[]): ScanResult["candidates"] {
  ...
  const contracts: Candidate[] = [];
  const quality: Candidate[] = [];
  const evidenceFiles = (keys: string[], fallback: string[] = []): string[] => unique(keys.flatMap((key) => entries[key]?.map((item) => item.path) ?? []).concat(fallback)).slice(0, 12);
  const push = (items: Candidate[], candidate: Candidate): void => {
    if (!items.some((item) => item.target === candidate.target)) {
      items.push(candidate);
    }
  };
  const hasAnyEntry = (keys: string[]): boolean => keys.some((key) => entries[key]?.length);
  ...
  if (has(["goods", "商品"])) push(domains, { target: "knowledge/domains/goods-master.md", reason: "goods domain entry detected", source_files: evidenceFiles(["controller", "service", "docs"]), confidence: 0.72, category: "domain" });
  if (has(["precisionorder", "精准订货"])) push(domains, { target: "knowledge/domains/precision-order.md", reason: "precision order entry detected", source_files: evidenceFiles(["controller", "service", "docs"]), confidence: 0.72, category: "domain" });
  if (has(["hddatasync", "hd", "海鼎"])) push(workflows, { target: "knowledge/workflows/hd-sync.md", reason: "HD sync entry detected", source_files: evidenceFiles(["tasks", "service", "docs"]), confidence: 0.7, category: "workflow" });
  if (has(["precisionorder", "精准订货"])) push(workflows, { target: "knowledge/workflows/precision-order-flow.md", reason: "precision order workflow detected", source_files: evidenceFiles(["controller", "service", "docs"]), confidence: 0.7, category: "workflow" });
  if (has(["mall", "feign", "mq", "remote", "thirdapi"])) push(integrations, { target: "knowledge/integrations/external-systems.md", reason: "external integration entry detected", source_files: evidenceFiles(["feign", "mq", "remote", "service", "docs"]), confidence: 0.72, category: "integration" });
...
function buildEvidencePlan(candidates: ScanResult["candidates"]): EvidencePlanItem[] {
...
function requiredEvidenceTypes(candidate: Candidate): string[] {
...
function missingEvidenceTypes(required: string[], files: string[]): string[] {
  return required.filter((type) => !files.some((file) => fileMatchesEvidenceType(file, type)));
}

function fileMatchesEvidenceType(file: string, type: string): boolean {
  if (type === "readme") return /(^|\/)README\.md$/i.test(file);
  if (type === "build_config") return /(^|\/)(package\.json|pom\.xml|tsconfig\.json)$/.test(file);
  if (type === "cli_entry") return /(^|\/)(index|cli|main)\.[cm]?[jt]s$|bin\//.test(file);
  if (type === "mcp_entry") return /(^|\/)(mcp|server)\.[cm]?[jt]s$|Mcp.*\.java$/.test(file);
  if (type === "adapter_command") return /\/commands\//.test(file);
  if (type === "adapter_tool") return /\/tools\//.test(file);
  if (type === "schema") return /^schema\/.*\.json$/.test(file);
  if (type === "tests") return /(^|\/)(test|tests|__tests__)\/|\.test\.[cm]?[jt]s$|\.spec\.[cm]?[jt]s$|Test\.java$/.test(file);
  if (type === "docs" || type === "release_docs") return /(^|\/)(README|CHANGELOG|CONTRIBUTING|SECURITY|AGENTS)\.md$|^docs\//i.test(file);
  if (type === "source_file" || type === "external_evidence") return Boolean(file);
  return false;
}

function externalEvidenceWarnings(repo: string, items: ExternalEvidenceItem[]): CheckItem[] {
...
function evidenceWarning(pathValue: string, ruleId: string, message: string, suggestion: string): CheckItem {
...
function isSensitiveConfigCandidate(rel: string): boolean {
  return (
    /(^|\/)(application|bootstrap|config).*\.(ya?ml|properties|json)$/.test(rel) ||
    /(^|\/)\.env(\..*)?$/.test(rel) ||
    /(^|\/)\.npmrc$/.test(rel) ||
    rel.includes("/config/") ||
