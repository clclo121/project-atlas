...
async function applyAllProposals(repo: string, rl: readline.Interface, yesAll: boolean): Promise<void> {
...
function loadProposal(repo: string, proposalId: string): Proposal {
  return readJson<Proposal>(path.join(proposalRoot(repo), proposalId, "proposal.json"));
}

function proposedProposals(repo: string): Proposal[] {
  ...
  return readdirSync(root, { withFileTypes: true })
    ...
    .map((entry) => {
      ...
      return existsSync(proposalPath) ? readJson<Proposal>(proposalPath) : null;
    })
    .filter((proposal): proposal is Proposal => proposal !== null && proposal.proposal_status === "proposed")
...
function budgetContextItems(items: ContextItem[], budget: number): ContextItem[] {
...
function checkKnowledge(repo: string): CheckResult {
  ...
  if (existsSync(path.join(repo, ".opencode/kb-proposals"))) {
    items.push(
      checkIssue(
        "warning",
        "legacy_opencode_proposals",
        ".opencode/kb-proposals",
        "legacy OpenCode proposal directory is present.",
        "Review or migrate pending proposals, then keep new proposals under .project-atlas/proposals.",
      ),
    );
  }
...
function assertSourceFilesExist(repo: string, sourceFiles: string[]): void {
...
function assertProposalContentHasNoFrontmatter(updates: UpdateInput["updates"]): void {
...
function validateFrontmatterScalar(value: string, label: string): string {
  if (/[\r\n\0]/.test(value)) {
